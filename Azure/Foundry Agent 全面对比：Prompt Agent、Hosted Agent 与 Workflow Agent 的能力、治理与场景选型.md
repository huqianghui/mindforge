...
> 姊妹篇（语音入口视角）：[[Azure Voice Live API 架构演进：与 Agent Service 解耦后的合作模式与组合选型]]。
...
| **治理** | **Strong**：RBAC、Audit、Monitoring、Prompt Shield、Content Filter、Cost Tracking 全在 portal 开箱即用 | **Medium**：Foundry 只看得见 Input/Output/Container 边界；内容安全、prompt 防护需在代码内自接 |
...
## 四、治理与安全：托管红利 vs 自建责任

治理是两者差距最大、也最容易被低估的维度：

### Prompt Agent 的托管红利

- **身份与权限**：RBAC 原生集成，portal 内管理。
- **安全防线**：Prompt Shield（注入防护）、Content Filter（内容安全）内置在运行时里，不需要写一行代码。
- **审计与成本**：Audit log、Token 级 Cost Tracking 开箱即用——这对企业合规（谁在什么时候用 Agent 做了什么、花了多少钱）是刚需。

### Hosted Agent 的自建责任清单

Foundry 对 Hosted Agent 的可见性止于容器边界（进什么、出什么、容器健康状态），以下都要自己做：

1. **内容安全**：在代码里显式调用 Azure AI Content Safety 或自建过滤；
2. **注入防护**：prompt 注入检测自己接；
3. **内部可观测性**：协议库给了基础 OTel 骨架（`/readiness`、OTLP export、graceful shutdown 都由 `azure-ai-agentserver-core` 继承而来），但 planner 每一步的决策、中间 LLM 调用要自己埋 span；
4. **成本追踪**：跨模型（尤其第三方模型）的 token 成本聚合自己算；
5. **密钥治理**：好消息是容器用专属 Entra agent identity 认证 Foundry 生态（模型、工具、AI Search），代码里可以做到零 API key；但你自己引入的外部依赖（第三方模型 API、自建数据库）的密钥管理归你。

**治理视角的选型直觉**：如果组织有强合规要求（金融、医疗、政府），Prompt Agent 的开箱治理本身就是决策权重很高的理由——用 Hosted Agent 复刻同等治理水位的工程成本常被严重低估。
...
姊妹篇[[Azure Voice Live API 架构演进：与 Agent Service 解耦后的合作模式与组合选型|Voice Live 文章]]讲了 Voice Live 挂 Agent 的模式三。一个自然的追问：**这个 `agent_id` 绑定对 Hosted Agent 也有效吗？**
...
2. 治理是最容易被低估的分水岭：Prompt Agent 的 RBAC/Prompt Shield/Content Filter/Cost Tracking 全托管；Hosted Agent 只有容器边界可见性，安全与可观测性内层全部自建。
...
- 相关笔记：[[Azure Voice Live API 架构演进：与 Agent Service 解耦后的合作模式与组合选型]]、[[Voice-Live-Agent实现架构——从级联流水线到Azure-Voice-Live-API]]
