...
表中"Planner + Executor 分层"、"技能原语"、"Bounded autonomy"这几行合起来，其实指向同一个更深的问题：**软件 Agent 那个"调模型 → 调工具 → 输出结束"的 loop，在具身系统里变成了什么？** 答案是被物理时间拆成三层不同频率的嵌套循环（任务层 0.1–1 Hz、策略层 10–50 Hz、控制层 100–1000 Hz），外加一个绕开模型的安全监视器——完整对比（含结束信号、工具性质、观测性质等五个本质分野）见[系列10](具身智能系列10：Agent循环的物理形态——分层多频率控制回路与五个本质分野.md)。
...
- **Agent 与系统结构簇**：[系列10](具身智能系列10：Agent循环的物理形态——分层多频率控制回路与五个本质分野.md) 软件 Agent 的 loop 在具身系统里变成什么——三层多频率控制回路、10 倍时间尺度分离原则与五个本质分野；[系列11](具身智能系列11：角色与实现的分离——七个功能角色、合并拆分光谱与扫地机器人的演进路径.md) 角色与实现的分离——七个功能角色、合并拆分光谱与扫地机器人的演进路径（按功能角色拆系统，与系列10 的时间频率拆解正交）
- **生成方法基础簇**：[系列12](具身智能系列12：扩散模型基础——从去噪机制到高维连续多峰的选型逻辑.md) 扩散模型基础——为什么世界模型和 VLA 都选了同一个生成方法：去噪机制与 score、高维/连续/多峰三属性的精确含义（与 LLM 逐一对照）、flow matching 与代价边界，是 VLA 簇与世界模型簇共同的方法论地基

阅读顺序建议按编号：世界模型簇的架构讲解（"跨时间自回归、帧内扩散"）依赖 VLA 簇建立的扩散头概念。系列10/11 是本篇概念映射表的纵深展开（分别沿时间维和功能维），只依赖本篇，可以在读完 01 后直接跳读。系列12 是 02/03/04 的机制纵深——"为什么两处都用扩散"讲的是同一个答案，适合在读完 03 或 04 后插读。后续视学习进展扩展灵巧操作、空间智能落地等主题。
