...
> VLA系列导航：[01 VLA输出架构演进](VLA系列01：VLA输出架构演进——从动作token到扩散动作头.md) · [02 扩散与Transformer的分工](VLA系列02：扩散与Transformer在VLA中的分工——动作块并行去噪与闭环重规划.md) · **03（本篇）** ｜ 总导读：[具身智能全景](具身智能全景：从LLM、Agent、RAG到物理世界闭环.md)
