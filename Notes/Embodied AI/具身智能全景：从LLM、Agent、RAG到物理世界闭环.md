...
> 本文是具身智能系列的总导读。五条主线（空间智能 / 世界模型 / 机器人系统 / VLA / 落地实践）的展开与推荐阅读顺序，见文末「阅读地图」。
...
本系列按主线组织成五个子系列，主线按分层栈自下而上排列（空间 → 认知 → 系统 → 行动 → 落地），编号只在线内有意义，**线内顺序即推荐阅读顺序**：

![具身智能系列阅读地图|720](../../asset/embodied-reading-map-2026-09-20.svg)

- **空间智能系列（空间层）**：[01 3D重建与分层栈](空间智能系列01：3D感知重建与世界模型的关系——状态与状态转移的空间智能分层栈.md)——状态 vs 状态转移 → [02 显式与隐式空间表达](空间智能系列02：显式与隐式空间表达——VGGT、3DGS到空间记忆的五层抽象与选用边界.md)——VGGT/3DGS 到空间记忆 → [03 3D内容生产的两条路线](空间智能系列03：3D内容生产的两条路线——Agent操作Blender与神经3D生成的分工.md) → [04 神经3D生成的开源版图](空间智能系列04：神经3D生成的开源版图——HY-World%202.0可探索世界、TRELLIS.2高保真资产与Marble对照.md)——HY-World 2.0 / TRELLIS.2 对标 Marble；
- **世界模型系列（认知层）**：[01 四条实现路线](世界模型系列01：世界模型不是一种架构——像素、潜空间、显式3D与物理方程四条实现路线.md)——像素/潜空间/显式3D/物理方程 → [02 与LLM的分野](世界模型系列02：世界模型与LLM的分野——动作条件化、空间持久记忆与中间件角色.md) → [03 与RL的边界](世界模型系列03：世界模型与强化学习的边界——判别三要素、脑内模拟器与特斯拉FSD的归类.md) → [04 潜空间与V-JEPA 2](世界模型系列04：潜空间路线与V-JEPA%202——预测表征而非像素、与Qwen3-VL的分野及潜空间验证五步法.md)——潜空间路线深潜，与 Qwen3-VL/VLA 的分工及潜空间验证 → [05 像素路线与Cosmos](世界模型系列05：像素路线与Cosmos——扩散与自回归双轨WFM、预训练到后训练的场景分工及全模态Cosmos%203.md)——像素路线深潜，双轨 WFM、预训练/后训练场景分工到全模态 Cosmos 3；
...
- **VLA系列（行动层）**：[01 输出架构演进](VLA系列01：VLA输出架构演进——从动作token到扩散动作头.md)——为什么 structured output 思路在动作输出上不成立 → [02 扩散与Transformer的分工](VLA系列02：扩散与Transformer在VLA中的分工——动作块并行去噪与闭环重规划.md)——"动作有因果依赖怎么还能并行生成" → [03 扩散模型基础](VLA系列03：扩散模型基础——从去噪机制到高维连续多峰的选型逻辑.md)——去噪机制与高维/连续/多峰选型逻辑，同时是世界模型线的方法论地基，适合在读完本线 02 或世界模型线 01 后插读 → [04 π0、FAST与Hi Robot](VLA系列04：π0、FAST与Hi%20Robot——流匹配动作专家、频域动作分词与分层交互.md) 与 [05 π0.5、知识绝缘与实时分块](VLA系列05：π0.5、知识绝缘与实时分块——开放世界泛化、梯度隔离与异步执行.md)——以 Physical Intelligence π 系列（开源 openpi）为样本，把前三篇的原理对到真实模型的完整演进线上；
...
1. **概念纵深**：本篇 → 世界模型系列 01→02→03→04→05 → 机器人系统系列 03（角色与实现的分离）；
2. **空间智能**：世界模型系列 01 → 空间智能系列 01→02→03→04；
