...
十步之后得到一条**整体自洁的曲线**，"先转弯后停止"的顺序自然包含在里面——因为训练数据里所有轨迹都是先转弯后停止，模型学到的分布里**不存在"先停后转"这种形状**。时序依赖没有被忽视，它被编码进了分布本身。
...
一句话总结：**块内的时序依赖靠双向注意力加迭代去噪来联合满足，对环境变化的因果响应靠块间重规划来满足——两者分工，谁都没有被牺牲。**
