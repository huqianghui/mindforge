...
> **RTK 系列文章**
> 1. **本文：RTK——AI Coding Agent 的 Token 压缩利器**（核心理念与使用方法）
...
**模块化架构**：RTK 为 67+ 种命令各自编写了**专用的过滤模块**，覆盖 Git、Rust、JavaScript、Python、Go、Docker 等 10 个生态系统，而非使用通用正则。这是 RTK 压缩率远超通用方案的关键。

> 详细的源码架构、模块结构和过滤策略实现，请参阅 [系列第二篇：源码深度解析](RTK系列02：源码深度解析——从CLI代理到Token压缩的工程实现.md)。
...
上述四大核心策略在实际实现中组合为多种具体模式，覆盖从 git 到 docker 的 100+ 命令，典型压缩率 50-99%。

> RTK 源码中共实现了 10 种过滤策略（逐行正则、状态机解析、JSON 注入、NDJSON 流式处理、分段过滤、多命令组合、去重、摘要分组、格式模板注入、TOML DSL 管道），每种策略的详细机制和适用场景，请参阅 [系列第二篇：源码深度解析](RTK系列02：源码深度解析——从CLI代理到Token压缩的工程实现.md)。
...
- [RTK 系列 02：源码深度解析](RTK系列02：源码深度解析——从CLI代理到Token压缩的工程实现.md)
- [RTK 系列 03：个性化优化引擎](RTK系列03：个性化优化引擎——基于Session数据的智能调优.md)
