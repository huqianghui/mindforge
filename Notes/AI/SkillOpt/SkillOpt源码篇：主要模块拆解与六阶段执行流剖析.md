...
> 承接 [[2026-07-01-SkillOpt]]（论文精读）与 [[SkillOpt实战篇：从AML+Azure OpenAI到SearchQA跑通文本空间skill训练|实战篇]]（跑通 runbook）。本篇对标 [[Agent Lightning系列02：框架全景与脊柱拆解——9大模块与method-agnostic设计|agent-lightning 系列 02]] 的"框架全景 + 脊柱拆解"，把 SkillOpt 仓库的**主要模块、核心函数、执行数据流**逐一读通。所有论断均对 `github.com/huqianghui/SkillOpt` 实际源码逐行核对，标注 `file:function`。
>
...
- **动手跑通**：环境/runbook/踩坑/实测数据 → [[SkillOpt实战篇：从AML+Azure OpenAI到SearchQA跑通文本空间skill训练]]
