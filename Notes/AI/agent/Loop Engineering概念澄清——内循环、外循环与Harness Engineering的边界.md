...
### 最小内核：外循环说到底是"自动发现 + workflow"

框架栈看起来很重，但剥到最里层，外循环的本质只有一个极简方程：

```
自动发现（auto-discovery） + workflow + 重复（cadence） + 跨轮记忆（state） = 外循环
```

四个零件缺一不可，但其中**自动发现是真正的分水岭**——它决定了"任务从哪来"，也就是"那个不断 prompt 的人"到底有没有被替换掉：

| | 任务从哪来 | 是不是外循环 |
|---|----------|------------|
| 普通 Harness / Ralph | 人喂进来 | ❌ 人还在轮次之间 |
| 外循环 | 系统自己 triage 发现 | ✅ 人退到设计者位置 |

把这个方程记住，后面所有"框架到底必不必要"的问题都能一眼判断：框架只是把这四个零件做得更稳、更并行、更可观测，**它们不构成新的本质**。

### 七个落地配方：Cobus Greyling 的 playbook

[Loop Engineering（Cobus Greyling）](https://cobusgreyling.github.io/loop-engineering/) 把外循环拆成 7 个可直接抄的 playbook，每个都是"一个触发器 + 一条窄 workflow + 一处记忆"的最小组合：

| Playbook | 触发 | 干什么 |
|----------|------|--------|
| **PR Babysitter** | PR 开启/更新 | 盯 CI、回应 review comment、推修复 |
| **Daily Triage** | 每日定时 | 扫 issues/CI/commits，整理出今日待办 |
| **CI Sweeper** | CI 失败 | 自动诊断红色构建并尝试修复 |
| **Post-Merge Cleanup** | merge 后 | 删分支、更 changelog、关联 issue |
| **Dependency Sweeper** | 每周定时 | 扫过期依赖，开升级 PR |
| **Issue Triage** | 新 issue | 打标签、分派、补复现步骤 |
| **Changelog Drafter** | release 前 | 从 commit 历史起草 changelog |

**关键澄清**：这 7 个不是"小循环（内循环）"，而是**窄范围（narrow-scope）的外循环**。这里有两条彼此正交的轴别混了——

- **高度轴**：内循环（单任务收敛）vs 外循环（自动发现→派发→验证）
- **范围轴**：宽/单体（一个循环管整个 SDLC）vs 窄/单一职责（一个循环只管一件事）

7 个 playbook 全部具备"自动发现 + workflow + 记忆"，所以都是外循环；它们只是**范围窄**——一个只盯 PR，一个只扫依赖。窄不等于矮：它们仍然替换掉了"那个盯着 PR、手动扫依赖的人"。

### 触发器 vs 控制流：cron job 与 dynamic workflow 是两根正交的轴

要 DIY 一个外循环，绕不开两个原语：cron job 和 dynamic workflow。它们常被混为一谈，但分管两件完全不同的事：

| | cron job | dynamic workflow |
|---|----------|-----------------|
| **管的是** | WHEN（何时触发） | HOW（如何决策执行） |
| **轴** | 触发/调度轴 | 控制流轴 |
| **谁决定** | 人预先写死（确定性调度） | 模型在运行时决定（agent 范式） |
| **对应 Anthropic 分类** | workflow（预定义路径） | agent（模型自主控制流） |
| **类比** | 闹钟 | 司机 |

一句话：**cron 决定"什么时候醒"，dynamic workflow 决定"醒了之后怎么干"**。外循环需要两者——光有 cron 没有 workflow，醒了不知道干嘛；光有 workflow 没有 cron，没人按下开始。

### DIY 最小外循环：cron + dynamic workflow 能平替框架吗？

能——**cron + dynamic workflow 就是一个最小可用外循环（MVP）**，足以平替轻量场景。把它对 Osmani 五件套做覆盖度盘点：

| Osmani 零件 | cron + dynamic workflow 能否平替 | 说明 |
|------------|------------------------------|------|
| **Automations** | ✅ 免费拿到 | cron 触发 + workflow 第一步 triage |
| **Skills** | ✅ 免费拿到 | workflow 里 `@SKILL.md` 注入 |
| **Connectors** | ✅ 免费拿到 | workflow 里调 MCP / `gh` / API |
| **Sub-agents** | ⚠️ 部分 | 能在 workflow 里拆 maker/checker，但要自己保证独立性 |
| **Worktrees** | ❌ 要手搓 | 得自己 `git worktree add` 做隔离 |
| **Memory** | ❌ 要手搓 | 得自己写 state.md 并在下一轮读回 |

落到 shell 上，最小外循环就是这么一条链：

```
cron（每日触发）
  → claude -p "扫 CI/issues，挑一个任务"          # 自动发现
    → git worktree add ../wt-$task                # 隔离（手搓）
      → maker: claude -p "在 worktree 里修到 test 过"  # 内循环
        → checker: claude -p "独立验证 diff"          # maker/checker 分离
          → echo "$result" >> state.md            # 记忆（手搓）
            → gh pr create                        # 集成
```

**结论**：框架（Conductor/Gas Town 等）省的不是"能不能跑"，而是 **worktree 隔离、state 持久化、并行调度、diff 审查这几件脏活**。任务量小、并行度低时，cron + dynamic workflow 完全够用；当你要同时跑几十个 Agent、要 one-click merge、要可观测面板时，才需要上框架。这正好印证了那条"瓶颈是基础设施，不是智能"——平替的边界，就卡在你愿意手搓多少基础设施上。
...
- [Loop Engineering](https://cobusgreyling.github.io/loop-engineering/)（Cobus Greyling）— 外循环的 7 个可抄 playbook（PR Babysitter、Daily Triage、CI Sweeper 等）
